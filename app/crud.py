...
def get_comments_by_situation(db: Session, situation_id: int):
    return db.query(models.Comment).filter(models.Comment.situation_id == situation_id).all()

def create_comment(db: Session, comment: schemas.CommentCreate, user_id: int, sentiment_analysis: dict = None):
...
def get_reactions_by_situation(db: Session, situation_id: int):
    return db.query(models.Reaction).filter(models.Reaction.situation_id == situation_id).all()
...
def delete_reaction(db: Session, situation_id: int, user_id: int, reaction_type: str):
    db_reaction = db.query(models.Reaction).filter(models.Reaction.situation_id == situation_id, models.Reaction.user_id == user_id, models.Reaction.reaction_type == reaction_type).first()
    if db_reaction:
