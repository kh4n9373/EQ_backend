...
def encrypt_refresh_token(refresh_token: str):
    return jwt.encode({"refresh_token": refresh_token}, SECRET_KEY, algorithm=ALGORITHM)


def decrypt_refresh_token(encrypted_token: str):
    payload = jwt.decode(encrypted_token, SECRET_KEY, algorithms=[ALGORITHM])
...
def get_new_access_token(user_id: int, db: Session):
    ...
    try:
        ...
        crud.update_user(
            db, db_user=user, data_to_update={"encrypted_refresh_token": None}
        )
        return None
